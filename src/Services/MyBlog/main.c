...
void * editorUpload_callback(struct AmmServer_DynamicRequest  * rqst)
{
    ...
    if ( _POST_OLD(rqst,"user",user,1024)  )      { AmmServer_Success("User=`%s`", user);     ++thingsSubmited; } else
                                                             { AmmServer_Success("did not find a user"); }

    if ( _POST_OLD(rqst,"token",token,1024)  )    { AmmServer_Success("Token=`********`");     ++thingsSubmited; } else
                                                             { AmmServer_Success("did not find a token"); }

    if ( _POST_OLD(rqst,"title",title,1024)  )    { AmmServer_Success("Title=`%s`", title);     ++thingsSubmited; } else
                                                             { AmmServer_Success("did not find a title"); }

    if ( _POST_OLD(rqst,"tags",tags,1024)  )      { AmmServer_Success("Tags=`%s`", tags);       ++thingsSubmited; } else
                                                             { AmmServer_Success("did not find tags"); }


    if ( _POST_OLD(rqst,"myDoc",bodyText,4096)  ) { AmmServer_Success("Text=`%s`", bodyText);   ++thingsSubmited; } else
