  ...
  if ( getTemperatureAndHumidityFromRequest(rqst,&temperature,&humidity) )
   {
     ...
     char message[512];
     snprintf(message,512,"This message has been generated by a button press on the sensor @ %u/%u/%u %u:%u:%u Room Temp:%0.2f / Humidity: %0.2f",
              ptm->tm_mday,ptm->tm_mon,EPOCH_YEAR_IN_TM_YEAR+ptm->tm_year,ptm->tm_hour,ptm->tm_min,ptm->tm_sec,temperature,humidity);
