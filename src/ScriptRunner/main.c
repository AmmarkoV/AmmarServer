...
void execute(char * command,char * param)
{
  ...
  { ///bin/bash -c \"
    ...
    if (strcmp(param,"wake")==0) { strcpy(commandToRun,"/bin/bash -c \"rostopic pub /ActionSequence HobbitMsgs/Command \"command: 'C_WAKEUP'\" -1\"  "); } else
    if (strcmp(param,"sleep")==0) { strcpy(commandToRun,"/bin/bash -c \"rostopic pub /ActionSequence HobbitMsgs/Command \"command: 'C_SLEEP'\" -1\"  "); } else
