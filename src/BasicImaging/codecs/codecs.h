...
//BasicImaging's CMakeLists.txt feature-detects libjpeg/libpng/libzstd+liblz4 ( find_package(JPEG)/
//find_package(PNG) , find_path+find_library for zstd/lz4 , which have no official CMake modules ) and
//passes USE_JPG_FILES/USE_PNG_FILES/USE_PZP_FILES in as a compiler definition accordingly , so unlike
//the upstream codecs library this header must NOT clobber a value that's already been defined on the
//command line ; the #ifndef guards are what let that override actually take effect. Everything else
//this trimmed-down vendored copy doesn't carry a reader/writer for at all , so it stays hardcoded off.
#ifndef USE_JPG_FILES
#define USE_JPG_FILES 1
#endif
#ifndef USE_PNG_FILES
#define USE_PNG_FILES 1
#endif
#ifndef USE_PZP_FILES
