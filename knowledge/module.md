...
| Logging | `tools/logs.c` | ✅ `system()` call in `compressLog()` audited (issues.md) — only ever called with server-configured log paths, no client input reaches it | ✅ found+fixed `pthread_self()%N` shard-hash bug (always shard 0) and unreliable exit()-flush from signal-handler context (issues.md) | | | ✅ `LOG_LINE_BUFFERED`/`LOG_SHARD_COUNT` (server_configuration.h) — sharded mutex recovered ~40-60% throughput at c=100/200, verified live (issues.md) |
